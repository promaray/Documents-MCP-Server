...
@mcp.prompt(
    name = "format",
    description = "Rewrite the contents of a document in a markdown format.",
)
def format_document(
    doc_id:str = Field(description="The ID of the document to format.")
) -> list[base.Message]:
    prompt = f"""
    Your goal is to reformat the contents of the document into a markdown format.
    The id of the document you need to reformat is:
     <document_id>
       {doc_id}
    </document_id>
   Add in headers, subheaders, and bullet points where appropriate. 
   Use the 'edit_document' tool to make any changes to the document's contents.
   """
    return [
